...
exports.logout = (req, res) => {
  req.logout();
  req.flash('success', 'You are now officially logged out!!');
  res.redirect('/');

};

exports.isLoggedIn = (req, res, next) => {
  //first chexk if the user is authenticated
  if (req.isAuthenticated()) {
    next(); //u r logged in so carry on
    return;
  };
  req.flash('error', 'Ooops you must be logged in to do that hehe!!!');
  res.redirect('/login');
